import React, {
  ...
  export const LogoLoop = React.memo<LogoLoopProps>(
    ({
      ...
      useEffect(() => {
        updateDimensions();
      }, [logoHeight, logos, gap]); // ← أعد الحساب عند تغير الحجم
      ...
      const updateDimensions = useCallback(() => {
        ...
        const seqWidth = seqRef.current?.getBoundingClientRect?.()?.width ?? 0;
      
        if (seqWidth > 0) {
          setSeqWidth(Math.ceil(seqWidth));
          // نضمن أن النسخ كافية جدًا
          const copiesNeeded = Math.ceil(containerWidth / seqWidth) + ANIMATION_CONFIG.COPY_HEADROOM;
          setCopyCount(Math.max(3, Math.ceil(copiesNeeded * 1.5)));
        }
